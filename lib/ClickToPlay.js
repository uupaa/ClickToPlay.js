...
//{@dev
    if (VERIFY) {
        ...
        $valid($type(order.video,    "UINT8|omit"),        ClickToPlay, "order.video");
        $valid($type(order.audio,    "UINT8|omit"),        ClickToPlay, "order.audio");
        $valid($type(order.sound,    "UINT8|omit"),        ClickToPlay, "order.sound");
...
//}@dev
    ...
    // --- stockyard (stocked resources) ---
    this._video = []; // [ HTMLVideoElement, ... ]
    this._audio = []; // [ HTMLAudioElement, ... ]
...
ClickToPlay["RESTRICTED"] = _isRestrictedAutoplay();
...
function _init(order, readyCallback, target) {
    ...
    for (i = 0, iz = videoCount; i < iz; ++i) {
        ...
        this._video[i].setAttribute("playsinline", true); // iOS 10+
    }
    ...
    } else if (ClickToPlay["RESTRICTED"]) { // iOS Safari, Chrome for Androdi 51+
    ...
    function _beSilent(event) { // @arg Node|Event
        ...
        that._sound.forEach(function(audioContext) {
            if (audioContext) {
                _playSilentSound(audioContext);
            }
        });
        that._ready = true;
...
function _playSilentSound(ctx) { // @arg AudioContext
